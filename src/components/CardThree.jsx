...
const CardThree = ({ notes, setNotes,selectedDocs }) => {
  ...
  setLoading(true);
  try {
    const response = await axios.post("http://localhost:8000/generate-mindmap", {
    ...
    console.error("Error generating mindmap", error.response?.data || error.message);
  }finally {
    setLoading(false);
  }
};
  ...
  if (type === 'Study Guide') contentEndpoint = 'http://127.0.0.1:8000/study-guide';
  else if (type === 'Briefing Doc') contentEndpoint = 'http://127.0.0.1:8000/briefing-doc';
  else if (type === 'FAQ') contentEndpoint = 'http://127.0.0.1:8000/faq';
  else return;

  const titleEndpoint = 'http://127.0.0.1:8000/get-note-title';
  ...
  try {
    ...
    if (!contentResponse.ok) throw new Error(`Content API error: ${contentResponse.status}`);
    ...
    if (!titleResponse.ok) throw new Error(`Title API error: ${titleResponse.status}`);
  ...
  <div className="card-three">
   ...
   >
      ...
      <div className="modal-overlay">
        ...
        >
          ...
          <div style={{ marginTop: '10px', textAlign: 'right' }}>
            <button onClick={() => setIsEditModalOpen(false)} style={{ marginRight: '10px', backgroundColor: 'blue' }}>
              Cancel
            </button>
            {notes[currentEditNoteIndex]?.editable && (
          <button onClick={handleSaveEdit} style={{ backgroundColor: 'green' }}>
           Save
          </button>
       )}
          </div>
    ...
    >
      ...
      <ReactMarkdown>{currentViewNote.content
      .replace(/\\n/g, '\n')
      .replace(/^"(.*)"$/, '$1')
      .replace(/^["']|["']$/g, '')    
      }</ReactMarkdown>
      <div style={{ textAlign: 'right', marginTop: '20px' }}>
        <button onClick={() => setIsViewModalOpen(false)} style={{backgroundColor: 'blue'}}>Close</button>
      </div>
     </div>
    </div>
   )}
